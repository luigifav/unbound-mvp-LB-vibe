...
import { CITIES_BY_UF, UF_LIST } from "@/lib/data/cities";
...
function Step3({ data, onChange, errors }: {
  ...
  const handleCEPChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (digits.length === 8) {
      cepTimerRef.current = setTimeout(() => lookupCEP(digits), 500);
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok && res.status !== 207) {
        ...
        throw new Error(
          detalhe
            ? `${data.mensagem || "Erro ao criar conta"}: ${detalhe}`
            : data.mensagem || "Erro ao criar conta"
        );
      }
